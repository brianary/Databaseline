...
.EXAMPLE
Initialize-DatabaseNotebook -ServerInstance ServerName -DatabaseName AdventureWorks
...
{
	'#r "nuget: Microsoft.DotNet.Interactive.SqlServer, *-*"' |Add-NotebookCell -Language csharp
}
...
Get-DbaDatabase -SqlInstance '$ServerInstance' -Database '$DatabaseName' |
	...
	Export-MermaidER |
	Add-NotebookCell -Language mermaid
"Last updated `$(Get-Date)"
"@ |Add-NotebookCell -Language pwsh #TODO: Add or replace dependency.
	if(!$Readme) {return}
	@"
``````mermaid
$(Get-DbaDatabase -SqlInstance $ServerInstance -Database $DatabaseName |
...
"@ |Add-NotebookCell -Language csharp
